...
X_eval = sm.add_constant(df_eval[["V", "weight"]])
y_true = df_eval["loss_rate"]
y_pred = inv_logit(loss_model.predict(X_eval))
...
file_pred = st.file_uploader("上传预测数据（长/宽/高/重量）", key="pred")

if file_pred and st.button("开始批量预测"):
    ...
    st.download_button("下载预测结果", buffer.getvalue(), "预测.xlsx")
...
file_train = st.file_uploader("上传训练数据", key="train")

if file_train and st.button("导入训练数据"):

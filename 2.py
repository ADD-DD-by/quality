...
# =========================
raw_data = [
    # L, W, H, weight, loss_rate, complaint_rate
...
df_init = pd.DataFrame(
    raw_data,
    columns=["L", "W", "H", "weight", "loss_rate", "complaint_rate"]
)

# 派生特征（统一口径）
df_init["V"] = df_init["L"] * df_init["W"] * df_init["H"]
df_init["girth"] = df_init["L"] + 2 * (df_init["W"] + df_init["H"])
df_init["weight_per_girth"] = df_init["weight"] / df_init["girth"]

FEATURE_COLS = ["V", "weight", "weight_per_girth"]
...
if "train_df" not in st.session_state:
    st.session_state.train_df = df_init.copy()
...
if st.button("🔍 评估运损风险", use_container_width=True):
    ...
    V = L * W * H
    girth = L + 2 * (W + H)
    weight_per_girth = weight / girth

    X_new = pd.DataFrame([{
        "const": 1,
        "V": V,
        "weight": weight,
        "weight_per_girth": weight_per_girth
    }])
    ...
    st.info(
        ...
        + (" 单位围长重量偏高；" if weight_per_girth >= 0.08 else "")
...
- **重量 / 围长系数：{coef['weight_per_girth']:.3f}**  
  → 描述结构“吃重”程度，越高越容易发生结构性破损
        """
    )
...
with st.form("add_train"):
    ...
    if st.form_submit_button("📥 添加并重训"):
        t_V = t_L * t_W * t_H
        t_girth = t_L + 2 * (t_W + t_H)
        t_weight_per_girth = t_weight / t_girth

        new_row = {
            "V": t_V,
            "weight": t_weight,
            "weight_per_girth": t_weight_per_girth,
            "loss_rate": t_loss,
...
st.divider()
st.caption(
    "⚠️ 本工具用于评估包装方案在不同体积、重量及结构吃重条件下的**相对资损/运损风险**，"
    "不用于精确预测单一订单的实际损失。"
)
